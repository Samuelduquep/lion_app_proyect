...
const TodoList: React.FC = () => {
    ...
    return (
        <div className="flex justify-between flex-col h-full p-3 bg-gradient-to-br from-blue-300 to-blue-500 rounded-md shadow-xl text-white">
            {/* Parte Superior */}
            <div className="flex-1 flex flex-col lg:flex-row lg:space-x-3 h-1-2">
                {/* Todo List */}
                <div className="w-full h-auto lg:w-1/2 bg-white text-gray-900 p-3 rounded-lg shadow-md border border-gray-200 lg:mb-0">
                    ...
                    <div className="mb-4">
                        <Input
                            ...
                            className="mb-2 rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50"
                            suffix={<PlusOutlined className="text-blue-500" />}
                        />
                        <Button
                            onClick={handleAddTodo}
                            type="primary"
                            icon={<PlusOutlined />}
                            className="w-full bg-blue-500 hover:bg-blue-600 text-white border border-transparent rounded-lg shadow-sm"
                        >
                            Add
                        </Button>
                    </div>
                    <List
                        bordered
                        className='overflow-auto max-h-64 pb-20'
                        dataSource={getStatusList('todo')}
                        renderItem={todo => (
                            <List.Item
                                key={todo.id}
                                className="flex items-center justify-between bg-white border-b border-gray-200 hover:bg-gray-50 transition duration-200"
                            >
                                <div
                ...
                {/* Chart */}
                <div className="w-full  h-full lg:w-2/3 bg-white p-6 rounded-lg shadow-md border border-gray-200">
                        ...
                        <Droppable key={status} droppableId={status}>
                            {(provided) => (
                                <div
                                    ...
                                    className="p-5 pb-10 w-1/3 h-1-4 bg-white rounded-lg shadow-md border border-gray-200"
                                >
                                    <div className="flex items-center justify-between">
                                        <h3 className="text-lg font-semibold capitalize text-gray-700">{status === 'todo' ? 'To Do' : status === 'doing' ? 'Doing' : 'Done'}</h3>
